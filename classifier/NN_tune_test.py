...
from utils import (calculate_performace, categorical_probas_to_classes,
                   create_dataset)

# Model architecture

input_1 = Input(shape=(1319)) #imput dimension
...
if __name__ == "__main__":
    ...
    for ((train, valid), k) in zip(skf.split(x_train, y_train), range(k_fold)):
        ...
        y_te = encoded_labels[valid]
        y_val= np_utils.to_categorical(y_te)
        print(y_val)
    ...
    # prediction probability in test set 
    print(X_test)
    print(X_test.shape)
    ...
    print(y_class)
    reverse = encoder.inverse_transform(y_class)
    print(reverse)
